...
class Service{
    ...
    async add(req,res){
        ...
        try {
                ...
                if(!verification){
                    ...
                    await model.work_keywords.create(msg).then((s) => {
                        data = {
                            status: 200,
                            data: s[0]
                        }
    ...
    async queryList(req,res){
        ...
        try {
            ...
            if(pageSize && page){
                item = await model.work_keywords.findAndCountAll({ where: where,limit:Number(pageSize), offset:(page - 1) * pageSize })
            }else{
                item = await model.work_keywords.findAll({ where: where })
            }
            data = {
                status: 200,
                data: item
    ...
    async update(req,res){
        ...
        try {
           ...
           update = this.emptyData(update)
                ...
                var item = await model.work_keywords.update(update,{
                    where: {
    ...
    async delete(req,res){
        ...
        try {
            ...
            await model.work_keywords.update({
                undefined:"undefined"
            },{
                where:deleteJson
            }).then((s) => {
    ...
    async query(req,res){
        ...
        try {
			...
			if (req.body.page && req.body.pageSize) {
                ...
                item = await model.work_keywords.findAndCountAll(query_data)
				item.page = req.body.page
				item.pageSize = req.body.pageSize
			}else{
                item = await model.work_keywords.findAll(query_data)
            }
